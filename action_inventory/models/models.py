...
class action_inventory(models.Model):
    ...
    roll_number = fields.Integer(string="Number Of Rolls/Boxes")
...
class action_inventory_scrap_fields(models.Model):
    ...
    scrap_reason = fields.Selection([
        ...
        ('انتهاء العمر الافتراضي للأبر', 'انتهاء العمر الافتراضي للأبر')], string="Reason Of Scrap")
    
    MO = fields.Char(string="MO_Reference")
    ...
    scrap_type = fields.Selection([
        ...
        ('ابرة مشرزة', 'ابرة مشرزة')], string='Type Of Scrap')
    
    scrap_lable = fields.Selection([
        ('Scrap', 'Scrap'),
        ('Black', 'Black'),
        ('White/Printing', 'White/Printing')], string='Lable ')
    notes = fields.Char(string="Notes")
...
class StockScrap(models.Model):
    ...
    worker = fields.Many2one(
                             'hr.employee', 'Responsable Worker',
                              domain="['|',('department_id', '=', 'Production'),('department_id', '=', 'Production / Maintenance')]")
